...
func (p Provider) GetToken(ctx context.Context, info clientauthenticationv1.ExecCredential) (clientauthenticationv1.ExecCredentialStatus, error) {
	...
	if err != nil {
		return clientauthenticationv1.ExecCredentialStatus{}, fmt.Errorf("failed to get secret %s/%s: %w", namespace, cfg.Name, err)
	}
	kubeconfigData, ok := sec.Data[cfg.Key]
	if !ok || len(kubeconfigData) == 0 {
		return clientauthenticationv1.ExecCredentialStatus{}, fmt.Errorf("secret %s/%s missing %q key", namespace, cfg.Name, cfg.Key)
	}

	// Parse kubeconfig
	config, err := clientcmd.Load(kubeconfigData)
	if err != nil {
		return clientauthenticationv1.ExecCredentialStatus{}, fmt.Errorf("failed to parse kubeconfig from secret %s/%s: %w", namespace, cfg.Name, err)
	}
	...
	if contextName == "" {
		return clientauthenticationv1.ExecCredentialStatus{}, fmt.Errorf("no context specified and no current-context in kubeconfig")
	}
	...
	if hasClientCert || hasClientKey {
		...
		if !hasClientCert {
			return clientauthenticationv1.ExecCredentialStatus{}, fmt.Errorf("client-key-data found but no client-certificate-data in user %q", context.AuthInfo)
		}
		if !hasClientKey {
			return clientauthenticationv1.ExecCredentialStatus{}, fmt.Errorf("client-certificate-data found but no client-key-data in user %q", context.AuthInfo)
		}
		...
		if len(user.ClientCertificateData) > 0 {
			...
			return clientauthenticationv1.ExecCredentialStatus{}, fmt.Errorf("client-certificate file path is not supported; use client-certificate-data")
		}
		...
		if len(user.ClientKeyData) > 0 {
			...
			// File path - not supported in this plugin
			return clientauthenticationv1.ExecCredentialStatus{}, fmt.Errorf("client-key file path is not supported; use client-key-data")
		}
	}

	// At least one authentication method must be present
	if status.Token == "" && status.ClientCertificateData == "" {
		return clientauthenticationv1.ExecCredentialStatus{}, fmt.Errorf("no authentication method found in user %q (neither token nor client-certificate-data/client-key-data)", context.AuthInfo)
	}
